...
public class Aeropuerto {
    ...
    public Integer cantDeVuelosQueLlegaronEn(LocalDate unDia) {
        return this.vuelosQueLlegaron.stream().filter(a -> a.fechaAproxLlegada().equals(unDia)).collect(Collectors.toList()).size();
    }

    public Integer cantDeVuelosQuePartieronEn(LocalDate unDia) {
        return this.vuelosQuePartieron.stream().filter(a -> a.fechaAproxLlegada().equals(unDia)).collect(Collectors.toList()).size();
    }
